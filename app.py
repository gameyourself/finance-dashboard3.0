...
# =========================
# 基础配置（Render Disk + seed 初始化版）
# =========================
BASE_DIR = Path(__file__).parent

# Render 上可通过环境变量指定持久化目录，本地则默认使用项目目录
DATA_DIR = Path(os.getenv("FINANCE_DATA_DIR", str(BASE_DIR / "finance_data_store")))
...
def initialize_data_store():
    """
    第一次启动时，如果持久化目录里没有 finance_db.json，
    但仓库 seed_data 里有，就复制过去。
    """
    try:
        if (not DB_FILE.exists()) and SEED_DB_FILE.exists():
            shutil.copy2(SEED_DB_FILE, DB_FILE)
    except Exception as e:
        print(f"[init] seed copy failed: {e}")
...
def backup_db():
    """
    每次保存前保留一份最新备份，防止误操作导致 JSON 损坏。
    """
    try:
        if DB_FILE.exists():
            backup_file = DATA_DIR / "finance_db_backup.json"
...
def local_finance_advice(user: dict, current_age: int, income_guess: float) -> str:
    ...
    derived_df = year_month_dataframe(user)
...
def generate_prediction_timeline(user: dict, event_flags: dict, progress_callback=None):
    ...
    history_df = year_month_dataframe(user)
...
def render_prediction_tab(user: dict, db: dict):
    ...
    age_df = year_month_dataframe(user)
    advice_income = 0.0
    if not age_df.empty:
...
with st.expander("部署诊断信息", expanded=False):
    st.write("BASE_DIR =", str(BASE_DIR))
    st.write("DATA_DIR =", str(DATA_DIR))
    st.write("SEED_DB_FILE =", str(SEED_DB_FILE))
    st.write("DB_FILE =", str(DB_FILE))
    st.write("seed exists =", SEED_DB_FILE.exists())
    st.write("db exists =", DB_FILE.exists())
    if DATA_DIR.exists():
        st.write("DATA_DIR files =", [p.name for p in DATA_DIR.iterdir()])
...
with st.sidebar:
    ...
    if st.button("创建新用户"):
        if new_user_name.strip():
            add_user(db, new_user_name.strip())
    ...
    st.divider()
    st.subheader("功能分类")
    function_category = st.radio("切换工作台", ["基础账本", "生命周期预测"], horizontal=False)

    st.divider()
    if st.button("手动全量重算当前用户"):
        if user_options and selected_user_id:
            current_user_for_recalc = user_map(db).get(selected_user_id)
            if current_user_for_recalc is not None:
                propagate_user_from(
                    current_user_for_recalc,
                    get_user_start_age(current_user_for_recalc),
                    1
                )
                save_db(db)
                st.success("当前用户已完成全量重算。")
                st.rerun()
        else:
            st.warning("请先选择一个用户。")
...
# 避免页面每次刷新时就执行超长年份的全量 propagate
if "initialized_user_ids" not in st.session_state:
    st.session_state["initialized_user_ids"] = set()

if selected_user_id not in st.session_state["initialized_user_ids"]:
    st.session_state["initialized_user_ids"].add(selected_user_id)
...
if function_category == "基础账本":
    ...
    with pred_tabs[8]:
        ...
        excel_path = DATA_DIR / f"{current_user['name']}_finance_export.xlsx"
        if st.button("生成当前用户 Excel", key="pred_generate_excel"):
            try:
                export_user_excel(current_user, excel_path)
                st.success(f"Excel 已生成：{excel_path.name}")
                st.rerun()
        ...
        if excel_path.exists():
            st.download_button("下载当前用户 Excel", data=excel_path.read_bytes(), file_name=excel_path.name, key="pred_download_excel")

        all_json = json.dumps(db, ensure_ascii=False, indent=2)
        st.download_button("下载全部数据库 JSON", all_json, file_name="finance_db_all_users.json", key="pred_download_all_json")
