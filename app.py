...
from analysis_engine import (
    build_current_group_factor_df as eng_build_current_group_factor_df,
    build_item_breakdown as eng_build_item_breakdown,
    compute_balance_factor_analysis as eng_compute_balance_factor_analysis,
    compute_cashflow_factor_analysis as eng_compute_cashflow_factor_analysis,
    slice_df_by_periods as eng_slice_df_by_periods,
)
...
def yearly_summary_dataframe(user: dict) -> pd.DataFrame:
    ...
    exclude_cols = ["期间", "说明", "决策说明", "月份"]
    numeric_cols = [c for c in df.columns if c not in exclude_cols]
    value_cols = [c for c in numeric_cols if c != "年份"]
    agg = df.groupby("年份")[value_cols].sum(numeric_only=True).reset_index()
    return agg
...
BADGE_IMAGE_CANDIDATES = {
    "大航海家": [
        r"D:\玛卡巴卡集合\大四下\当代金融教育\代码\Demo设计 2.0\大航海家.png",
        str(BASE_DIR / "大航海家.png"),
        str(BASE_DIR / "assets" / "大航海家.png"),
        "/mnt/data/24de26f7-201f-45f5-af04-1d616fbe0c05.png",
    ],
    "小航海家": [
        r"D:\玛卡巴卡集合\大四下\当代金融教育\代码\Demo设计 2.0\小航海家.png",
        str(BASE_DIR / "小航海家.png"),
        str(BASE_DIR / "assets" / "小航海家.png"),
        "/mnt/data/1d76f04e-506f-4f23-9d36-631ee7b274bb.png",
    ],
}
...
def find_badge_image_path(title: str) -> Optional[str]:
    for candidate in BADGE_IMAGE_CANDIDATES.get(title, []):
        try:
...
@st.cache_data(show_spinner=False)
def cached_balance_timeline_dataframe(user_snapshot: str) -> pd.DataFrame:
    user = json.loads(user_snapshot)
    rows = []
    ensure_user_timeline(user)
    for y in get_user_years(user):
        for m in MONTHS:
            ...
            row = {
                ...
                '说明': rec.get('note', ''),
                '决策说明': rec.get('decision_summary', ''),
                '总资产': float(rec['derived'].get('总资产', 0) or 0),
                '总负债': float(rec['derived'].get('总负债', 0) or 0),
                '净资产': float(rec['derived'].get('净资产', 0) or 0),
            }
            for item in [i for group in BALANCE_ASSETS.values() for i in group]:
                row[item] = float(rec['balance_sheet']['assets'].get(item, 0) or 0)
            for item in [i for group in BALANCE_LIABILITIES.values() for i in group]:
                row[item] = float(rec['balance_sheet']['liabilities'].get(item, 0) or 0)
            rows.append(row)
    return pd.DataFrame(rows)


def get_year_slice(df: pd.DataFrame, year: int) -> pd.DataFrame:
    work = df[df['年份'] == year].copy()
    if work.empty:
        return df.copy()
    return work


def get_time_window_options():
    return ['近3个月', '近6个月', '近1年', '本年', '全部历史']


def get_time_window_keys(user: dict, end_year: int, end_month: int, window_label: str) -> List[str]:
    valid_keys = [month_key(y, m) for y in get_user_years(user) for m in MONTHS]
    ...
    if window_label == '本年':
        return [month_key(end_year, m) for m in MONTHS if month_key(end_year, m) in valid_keys and m <= end_month]
...
def make_ref_table(df: pd.DataFrame, title: str):
    st.markdown(f'#### {title}')
    if df is None or df.empty:
        st.caption('暂无强关联月份记录。')
    else:
        st.dataframe(df, use_container_width=True, hide_index=True)
...
def latest_nonzero_months(df: pd.DataFrame, column: str, top_n: int = 3) -> pd.DataFrame:
    work = df[['期间', '年份', '月份', column, '决策说明', '说明']].copy()
    work = work[work[column].abs() > 1e-9]
    if work.empty:
        return work
    return work.assign(_abs=work[column].abs()).sort_values(['_abs', '月份'], ascending=[False, False]).drop(columns=['_abs']).head(top_n).reset_index(drop=True)


def build_group_breakdown(record: dict, side: str) -> pd.DataFrame:
    rows = []
    source = BALANCE_ASSETS if side == 'assets' else BALANCE_LIABILITIES
    values = record['balance_sheet'][side]
    total = sum(float(values.get(item, 0) or 0) for items in source.values() for item in items)
...
def build_item_breakdown(record: dict, side: str, group_name: str) -> pd.DataFrame:
    source = BALANCE_ASSETS if side == 'assets' else BALANCE_LIABILITIES
    values = record['balance_sheet'][side]
    items = source.get(group_name, [])
...
GROUP_CASHFLOW_LINK_HINTS = {
    ('assets', '流动性资产'): ['工资薪金', '劳务报酬', '经营所得', '养老金', '利息收入', '饮食开支', '居住成本', '通勤交通'],
    ('assets', '投资性资产'): ['利息收入', '金融资产变现损益', '实物资产变现损益', '工资薪金'],
    ('assets', '自用性资产'): ['居住成本', '通勤交通', '工资薪金'],
    ('assets', '其他资产'): ['其他所得', '偶然所得'],
    ('liabilities', '流动性负债'): ['利息费用', '其他财务支出', '饮食开支', '日常用品'],
    ('liabilities', '投资性负债'): ['利息费用', '金融资产变现损益', '经营所得'],
    ('liabilities', '自用性负债'): ['利息费用', '居住成本', '通勤交通', '工资薪金'],
    ('liabilities', '其他负债'): ['其他财务支出', '其他无法归类支出'],
}


def build_current_group_factor_df(user: dict, year: int, month: int, side: str, group_name: str) -> tuple[pd.DataFrame, str]:
    record = get_record(user, year, month)
    item_df = build_item_breakdown(record, side, group_name)
    ...
    snapshot = get_user_snapshot(user)
    cf_df = get_year_slice(cached_year_month_dataframe(snapshot), year)
        ...
        for hint in hints:
            ...
            for lvl1, lvl2, lvl3 in flatten_cashflow_items():
                if lvl3 == hint:
                    ...
                    if maybe in cf_df.columns:
                        col = maybe
                        break
            if col and cf_df[col].abs().sum() > 0:
                linked.append((hint, float(cf_df[col].abs().mean())))
    ...
    advice = f'当前{group_name}内部主要由 ' + '、'.join([f"{r['因子']}（{r['当前占比']*100:.1f}%）" for _, r in pd.DataFrame(rows).head(3).iterrows()]) + ' 构成。'
    if link_texts:
        advice += ' 结合现金流看，' + '；'.join(link_texts[:3]) + '。'
    return pd.DataFrame(rows).sort_values('结构贡献分', ascending=False).reset_index(drop=True), advice


def compute_balance_factor_analysis(user: dict, year: int, month: int, target_metric: str = '净资产') -> dict:
    snapshot = get_user_snapshot(user)
    df = get_year_slice(cached_balance_timeline_dataframe(snapshot), year)
    record = get_record(user, year, month)

    asset_group_df = build_group_breakdown(record, 'assets')
    liability_group_df = build_group_breakdown(record, 'liabilities')

    target_series = df[target_metric] if target_metric in df.columns else df['净资产']
    rows = []
    for item in [i for group in BALANCE_ASSETS.values() for i in group if i in df.columns]:
        current_value = float(record['balance_sheet']['assets'].get(item, 0) or 0)
        corr = safe_corr(df[item], target_series)
        rows.append({
            '方向': '正向资产',
            '一级分类': next((g for g, its in BALANCE_ASSETS.items() if item in its), ''),
            '因子': item,
            '当前值': current_value,
            '当前占比': safe_ratio(current_value, max(float(record['derived'].get('总资产', 0) or 0), 1e-9)),
            '相关系数': corr,
        })
    for item in [i for group in BALANCE_LIABILITIES.values() for i in group if i in df.columns]:
        current_value = float(record['balance_sheet']['liabilities'].get(item, 0) or 0)
        corr = safe_corr(df[item], target_series)
        rows.append({
            '方向': '负向负债',
            '一级分类': next((g for g, its in BALANCE_LIABILITIES.items() if item in its), ''),
            '因子': item,
            '当前值': current_value,
            '当前占比': safe_ratio(current_value, max(float(record['derived'].get('总负债', 0) or 0), 1e-9)),
            '相关系数': corr,
        })
    factor_df = pd.DataFrame(rows)
    if factor_df.empty:
        factor_df = pd.DataFrame(columns=['方向', '一级分类', '因子', '当前值', '当前占比', '相关系数', '综合贡献分'])
        month_ref_df = pd.DataFrame()
    else:
        ...
        month_refs = []
        for _, row in factor_df.head(6).iterrows():
            factor = row['因子']
            if factor in df.columns:
                top_months = latest_nonzero_months(df, factor, top_n=2)
                for _, m_row in top_months.iterrows():
                    month_refs.append({
                        '因子': factor,
                        '期间': m_row['期间'],
                        '贡献值': float(m_row[factor] or 0),
                        '决策说明': m_row.get('决策说明', '') or '暂无',
                        '波动说明': m_row.get('说明', '') or '暂无',
                    })
        month_ref_df = pd.DataFrame(month_refs)

    positive = factor_df[factor_df['方向'] == '正向资产'].head(3)
    negative = factor_df[factor_df['方向'] == '负向负债'].head(3)
    ...
    summary = f'以下时间性影响因子仅基于 {year} 年的月度变化计算。当前{target_metric}更受 {pos_text} 支撑，同时要关注 {neg_text} 带来的拖累。'

    advice = []
    if not positive.empty:
        advice.append('正向资产因子可以继续保持，优先保护占比高且在本年内相关性更稳定的资产项目。')
    if not negative.empty:
        advice.append('负向负债因子建议先和本年现金流支出一起看，优先压降利息成本和高频负债。')
    if month_ref_df is not None and not month_ref_df.empty:
        refs = [f"{row['期间']}关于{row['因子']}的记录：{row['决策说明']}" for _, row in month_ref_df.head(3).iterrows()]
        advice.append('结合本年记录，' + '；'.join(refs) + '。')

    return {
        'asset_group_df': asset_group_df,
        'liability_group_df': liability_group_df,
        'factor_df': factor_df,
        'month_ref_df': month_ref_df,
        'summary': summary,
        'advice': ' '.join(advice),
    }


def compute_cashflow_factor_analysis(user: dict, year: int, month: int, target_metric: str = '储蓄') -> dict:
    snapshot = get_user_snapshot(user)
    df = get_year_slice(cached_year_month_dataframe(snapshot), year).copy()
    record = get_record(user, year, month)
    target_series = df[target_metric] if target_metric in df.columns else df['储蓄']
    ...
    for lvl1, lvl2, lvl3 in flatten_cashflow_items():
        ...
        rows.append({
            ...
            '相关系数': safe_corr(df[col], target_series),
        })
    factor_df = pd.DataFrame(rows)
    if factor_df.empty:
        factor_df = pd.DataFrame(columns=['方向', '一级分类', '二级分类', '因子', '当前值', '当前占比', '相关系数', '综合贡献分'])
    else:
        factor_df['综合贡献分'] = factor_df['当前占比'].abs() * 0.6 + factor_df['相关系数'].abs() * 0.4
        factor_df = factor_df.sort_values('综合贡献分', ascending=False).reset_index(drop=True)
    ...
    for _, row in factor_df.head(6).iterrows():
        ...
        for lvl1, lvl2, lvl3 in flatten_cashflow_items():
            if lvl3 == factor:
                candidate = f'{lvl1}/{lvl2}/{lvl3}'
                if candidate in df.columns:
                    target_col = candidate
                    break
        if target_col:
            top_months = latest_nonzero_months(df.rename(columns={target_col: factor}), factor, top_n=2)
            for _, m_row in top_months.iterrows():
                month_refs.append({
                    '因子': factor,
                    '期间': m_row['期间'],
    ...
    summary = f'以下现金流影响因子仅基于 {year} 年的月度变化计算。当前{target_metric}主要由 {pos_text} 拉动，同时被 {neg_text} 消耗。'

    advice = []
    if not income_df.empty:
        advice.append('收入侧建议优先保护稳定工资、利息类和持续性强的来源。')
    if not expense_df.empty:
        advice.append('支出侧建议先盯住占比高且本年波动也大的项目，优先压降可替代支出。')
    if month_ref_df is not None and not month_ref_df.empty:
        refs = [f"{row['期间']}关于{row['因子']}的记录：{row['决策说明']}" for _, row in month_ref_df.head(3).iterrows()]
        advice.append('结合本年月份记录，' + '；'.join(refs) + '。')
...
def make_pie_chart(df: pd.DataFrame, label_col: str, value_col: str, title: str, chart_scale: float = 0.58):
    work = df.copy()
    if work.empty:
        fig = go.Figure()
        fig.update_layout(title=title, height=420)
        return fig
    actual_values = work[value_col].astype(float).tolist()
    total = sum(actual_values)
    if total <= 0:
        display_values = [1 for _ in actual_values]
    else:
        epsilon = max(total * 0.0015, 0.5)
        display_values = [v if v > 0 else epsilon for v in actual_values]

    labels = work[label_col].astype(str).tolist()
    short_labels = [lbl if len(lbl) <= 10 else lbl[:10] for lbl in labels]
    fig = go.Figure(data=[go.Pie(
        labels=labels,
        values=display_values,
        customdata=list(zip(actual_values, short_labels)),
        hole=0.52,
        sort=False,
        textinfo='text+percent',
        text=[s for s in short_labels],
        textposition='outside',
        outsidetextfont=dict(size=10),
        automargin=True,
        hovertemplate='%{label}<br>展示占比:%{percent}<br>实际金额:%{customdata[0]:,.2f}<extra></extra>',
        domain=dict(x=[(1-chart_scale)/2, 1-(1-chart_scale)/2], y=[0.12, 0.88]),
        pull=[0.01 if float(v)==0 else 0 for v in actual_values]
    )])
    fig.update_layout(
        title=title,
        height=420,
        margin=dict(l=60, r=150, t=60, b=30),
...
def make_cash_bar_chart(df: pd.DataFrame, title: str):
    work = df.copy()
    if '展示名' not in work.columns:
        if '因子' in work.columns and '一级分类' in work.columns:
            work['展示名'] = work['一级分类'].astype(str) + '-' + work['因子'].astype(str)
        elif '因子' in work.columns:
            work['展示名'] = work['因子'].astype(str)
        else:
            work['展示名'] = work.index.astype(str)
    fig = go.Figure(data=[go.Bar(x=work['展示名'], y=work['当前值'])])
    fig.update_layout(title=title, xaxis_title='因子', yaxis_title='金额', margin=dict(l=10, r=10, t=50, b=10))
    return fig


def get_navigator_title_and_text(record: dict) -> tuple[str, str]:
    net_asset = float(record.get('derived', {}).get('净资产', 0) or 0)
    if net_asset >= 0:
        return '大航海家', '净资产为正，说明当前航线总体向上，可以继续巩固资产主引擎。'
    return '小航海家', '净资产暂时为负，说明航线仍在穿越风浪，重点要先压降高负债区。'


def safe_show_image(image_path, caption=None):
    try:
        ...
        try:
            ...
            st.image(image_path, caption=caption, width=320)


def render_assessment_card(record: dict):
    title, desc = get_navigator_title_and_text(record)
    badge_path = find_badge_image_path(title)
    st.markdown(f"<h2 style='text-align:center;margin-bottom:8px;'>航海称号：{title}</h2>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1, 2, 1])
    with c2:
        if badge_path:
            safe_show_image(badge_path)
        else:
            st.info(title)
    st.markdown(f"<div style='text-align:center;font-size:20px;font-weight:600;margin-top:6px;'>{desc}</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='text-align:center;margin-top:8px;'><b>当月决策说明：</b> {record.get('decision_summary', '') or '暂无'}<br><b>当月波动说明：</b> {record.get('note', '') or '暂无'}</div>", unsafe_allow_html=True)



def render_finance_navigator(user: dict):
    st.subheader('资产仪表盘')
    year_options = get_user_years(user)
    default_year = year_options[0]
    c1, c2, c3, c4 = st.columns([1, 1, 1.15, 1.2])
    year = c1.selectbox('分析年龄/年份', year_options, index=year_options.index(default_year), key='nav_year')
    month = c2.selectbox('分析月份', MONTHS, index=0, key='nav_month')
    mode = c3.radio('分析模式', ['资产负债贡献', '现金流贡献'], horizontal=True, key='nav_mode')
    window_label = c4.selectbox('分析时间范围', get_time_window_options(), index=3, key='nav_window')
    record = get_record(user, year, month)

    total_asset = float(record['derived'].get('总资产', 0) or 0)
    total_debt = float(record['derived'].get('总负债', 0) or 0)
    net_asset = float(record['derived'].get('净资产', 0) or 0)

    st.markdown(f"""
    <div style='display:flex;justify-content:center;align-items:flex-end;gap:24px;margin:10px 0 18px 0;'>
        <div style='text-align:center;'>
            <div style='width:140px;height:140px;border-radius:50%;background:linear-gradient(135deg,#93c5fd,#2563eb);display:flex;align-items:center;justify-content:center;color:white;font-size:24px;font-weight:700;box-shadow:0 10px 24px rgba(37,99,235,0.28);'>资产</div>
            <div style='margin-top:8px;font-weight:600;'>¥{fmt_money(total_asset)}</div>
        </div>
        <div style='text-align:center;'>
            <div style='width:170px;height:170px;border-radius:50%;background:linear-gradient(135deg,#86efac,#16a34a);display:flex;align-items:center;justify-content:center;color:white;font-size:28px;font-weight:700;box-shadow:0 12px 28px rgba(22,163,74,0.28);'>净资产</div>
            <div style='margin-top:8px;font-weight:700;'>¥{fmt_money(net_asset)}</div>
        </div>
        <div style='text-align:center;'>
            <div style='width:140px;height:140px;border-radius:50%;background:linear-gradient(135deg,#fca5a5,#dc2626);display:flex;align-items:center;justify-content:center;color:white;font-size:24px;font-weight:700;box-shadow:0 10px 24px rgba(220,38,38,0.28);'>负债</div>
            <div style='margin-top:8px;font-weight:600;'>¥{fmt_money(total_debt)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    snapshot = get_user_snapshot(user)
    cash_df_all = cached_year_month_dataframe(snapshot)
    balance_df_all = cached_balance_timeline_dataframe(snapshot)

    if mode == '资产负债贡献':
        result = eng_compute_balance_factor_analysis(
            balance_df_all,
            record,
            year,
            month,
        ...
        with pie_left:
            st.plotly_chart(make_pie_chart(result['asset_group_df'], '大项', '金额', '资产大项结构', chart_scale=0.56), use_container_width=True)
        with pie_right:
            st.plotly_chart(make_pie_chart(result['liability_group_df'], '大项', '金额', '负债大项结构', chart_scale=0.56), use_container_width=True)

        st.markdown('#### 资产负债核心判断')
        st.info(result['summary'])
        st.success(result['advice'])

        focus_col1, focus_col2 = st.columns([1, 1.4])
        group_focus = focus_col1.radio('查看对象', ['资产', '负债'], horizontal=True, key='nav_balance_focus')
        structure_level = focus_col2.radio('结构层级', ['大项结构', '子项结构'], horizontal=True, key='nav_structure_level')

        if group_focus == '资产':
            group_df = result['asset_group_df']
            source_groups = BALANCE_ASSETS
            side = 'assets'
        else:
            group_df = result['liability_group_df']
            source_groups = BALANCE_LIABILITIES
            side = 'liabilities'

        if structure_level == '大项结构':
            st.markdown(f'#### {group_focus}大项结构明细')
            detail_left, detail_right = st.columns([1.05, 1])
            with detail_left:
                st.dataframe(group_df, use_container_width=True, hide_index=True)
            with detail_right:
                st.plotly_chart(make_pie_chart(group_df, '大项', '金额', f'{group_focus}大项结构', chart_scale=0.56), use_container_width=True)
        else:
            chosen_group = st.selectbox(f'选择{group_focus}大项', group_df['大项'].tolist(), key='nav_group_select')
            item_df = eng_build_item_breakdown(record, side, chosen_group, source_groups)
            sub_left, sub_right = st.columns([1.05, 1])
            with sub_left:
                st.dataframe(group_df, use_container_width=True, hide_index=True)
                st.dataframe(item_df, use_container_width=True, hide_index=True)
            with sub_right:
                st.plotly_chart(make_pie_chart(item_df, '子项', '金额', f'{chosen_group}子项结构', chart_scale=0.56), use_container_width=True)

        asset_tab, debt_tab = st.tabs(['资产时间性影响', '负债时间性影响'])
        with asset_tab:
            asset_show = result['asset_factor_df'].drop(columns=['列名'], errors='ignore')
            st.markdown(f'#### 资产影响因子（{window_label}）')
            st.dataframe(asset_show, use_container_width=True, hide_index=True)
            if not result['asset_factor_df'].empty:
                st.plotly_chart(make_cash_bar_chart(result['asset_factor_df'].assign(展示名=result['asset_factor_df']['因子']), '资产影响因子'), use_container_width=True)
            make_ref_table(result['asset_month_ref_df'], f'资产强关联月份回看（{window_label}）')
        with debt_tab:
            debt_show = result['debt_factor_df'].drop(columns=['列名'], errors='ignore')
            st.markdown(f'#### 负债影响因子（{window_label}）')
            st.dataframe(debt_show, use_container_width=True, hide_index=True)
            if not result['debt_factor_df'].empty:
                st.plotly_chart(make_cash_bar_chart(result['debt_factor_df'].assign(展示名=result['debt_factor_df']['因子']), '负债影响因子'), use_container_width=True)
            make_ref_table(result['debt_month_ref_df'], f'负债强关联月份回看（{window_label}）')

    else:
        result = eng_compute_cashflow_factor_analysis(
            cash_df_all,
            record,
            year,
            month,
            window_label,
            flatten_cashflow_items(),
            target_metric='储蓄',
        )
        st.markdown('#### 现金流核心判断')
        st.info(result['summary'])
        ...
        income_tab, expense_tab = st.tabs(['收入分析', '支出分析'])
        with income_tab:
            income_show = result['income_df'].copy()
            st.markdown(f'#### 收入影响因子（{window_label}）')
            st.dataframe(income_show.drop(columns=['展示名', '列名'], errors='ignore'), use_container_width=True, hide_index=True)
            if not income_show.empty:
                st.plotly_chart(make_cash_bar_chart(income_show, '收入影响因子'), use_container_width=True)
            make_ref_table(result['income_month_ref_df'], f'收入强关联月份回看（{window_label}）')
        with expense_tab:
            expense_show = result['expense_df'].copy()
            st.markdown(f'#### 支出影响因子（{window_label}）')
            st.dataframe(expense_show.drop(columns=['展示名', '列名'], errors='ignore'), use_container_width=True, hide_index=True)
            if not expense_show.empty:
                st.plotly_chart(make_cash_bar_chart(expense_show, '支出影响因子'), use_container_width=True)
            make_ref_table(result['expense_month_ref_df'], f'支出强关联月份回看（{window_label}）')
...
st.caption("支持25-35岁基础记录、36-80岁生命周期预测、事件影响注入、AI生成、月度/年度访问、对比分析、储蓄联动、航海资产分析与用户转账。")
...
if function_category == "基础账本":
    main_tabs = st.tabs(["月度访问", "资产仪表盘", "年度与五年访问", "对比分析", "储蓄联动图", "人物背景", "AI生成/导入", "用户转账", "导出"])
    ...
    with main_tabs[1]:
        render_finance_navigator(current_user)
    ...
    pred_tabs = st.tabs(["生命周期预测", "月度访问", "资产仪表盘", "年度与五年访问", "对比分析", "储蓄联动图", "人物背景", "AI生成/导入", "用户转账", "导出"])
